...
# check the names of your variables. Are they ok or do they need updated?
names(deputies)

# check if any of the receipt values are missing, and if so, remove them. 
deputies %>%
  summarise(count = sum(is.na(receipt_value)))

# Spend some time visualizing any outliers in the "receipt_value" column. Does it have a relationship with the "receipt_description" column?
ggplot(deputies, aes(receipt_description, receipt_value)) + geom_boxplot() + coord_flip()


# identify which rows in the "receipt_value" column of data_v1 dataset are outliers. Try plotting them or looking through just the outliers. Do you notice anything interesting?
outlier_scores <- scores(deputies$receipt_value)
head(outlier_scores)

# calculate outlier scores for each 
...
# replot the data
ggplot(deputies, aes(receipt_description, receipt_value)) + 
  geom_boxplot() + 
  coord_flip() + 
  facet_wrap(~is_outlier)

# make an outliers data set
deputies_outliers <- deputies %>%
  filter(is_outlier == FALSE)
deputies_outliers

# make an dataset with no outliers
deputies_no_outliers <- deputies %>%
  filter(is_outlier == TRUE)
deputies_no_outliers

# find the most dodgy political parties
dodgy_claims <- deputies %>%
  select(political_party, receipt_description, receipt_value) %>%
  group_by(political_party, receipt_description) %>%
  summarise(all_receipts = sum(receipt_value)) %>%
  arrange(desc(all_receipts))
