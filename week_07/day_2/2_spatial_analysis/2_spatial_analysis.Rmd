...
<br>
Next we can create a custom grid to plot our data. For this, we can use the function `st_make_grid`. This allows you to create grids relating to spatial data. In our case, we will put our transformed data in, with a resolution of 50km (cellsize = 50000 - metres in our case). The resolution was found through trial and error of different sizes. And we include the call `what = ` to show what geometry we would like to create what (polygons, centers or corners).
...
<br>
That is our basic grid. We can then use the `st_intersection` function to extract the points in the limits of our country of choice, so we plot only the grid that overlaps with our region of interest. 
...
Why do we need to do this? Well, if we want to calculate distances between two points, we need to have them as lines instead of shapes. So if we convert to a line, we are able to measure distances from one point to another. For this example, we want to measure the distance between each point between the country outline (the coast) and 
